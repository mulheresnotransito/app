  ...
  return (
    <Styled.Container style={{ paddingTop: 0 }}>
      ...
      <Styled.ScrollContainer>
        ...
        {/* <View style={{ marginTop: 10, marginBottom: -5, width: '90%', alignItems: 'center', justifyContent: 'space-between', flexDirection: 'row', flexWrap: 'nowrap', }}>
          {((months.findIndex(selectedMonth) > 0 && months.length > 1) && selectedMonth == m) &&
            <TouchableOpacity onPress={() => setSelectedMonth(months[months.findIndex(selectedMonth) - 1])}>
              <Text>{"<"}</Text>
            </TouchableOpacity>
          }
          <Text style={{ flex: 1, textAlign: 'center', color: "#C43A57", fontSize: 18, fontWeight: '300' }}>{selectedMonth}</Text>
          {((months.findIndex(selectedMonth) >= 0 && months.length > 1) && selectedMonth == m) &&
            <TouchableOpacity onPress={() => setSelectedMonth(months[months.findIndex(selectedMonth) + 1])}>
              <Text>{">"}</Text>
            </TouchableOpacity>
